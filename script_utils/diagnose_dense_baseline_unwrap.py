...
def main():
    ...
    # v2: UNWRAP torch.compile, like the dense_self_sparsify_per_lh script does.
    # All other settings (canonical seed_everything per batch, natural B=6
    # single batch, same sampling_model_args) are identical to v1. Tests
    # whether compile-unwrap alone accounts for the script's baseline gap.
    if hasattr(model.nn, "_orig_mod"):
        logger.info("  UNWRAPPING torch.compile (v2 diagnostic)")
        model.nn = model.nn._orig_mod
    logger.info(
        f"  model.nn type: {type(model.nn).__name__}"
    )

    out_root = Path("results/diagnose_dense_baseline/canonical_seed_n6_L50100_unwrap")
