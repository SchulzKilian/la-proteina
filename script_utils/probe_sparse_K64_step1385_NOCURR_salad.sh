#!/bin/bash
# Inference-only schedule-sensitivity probe: K=64 curriculum-self ckpt loaded
# with curriculum FORCED OFF → static SALAD-canonical (16/16/32, K=64) at all t.
# Distribution-shift test, not a clean architectural comparison — see the
# inference YAML header for the caveat.
#
# N=6 × L∈{50, 100, 200} × nsteps=400. ~30 min on 1× A100.
# Compare designability vs `probe_sparse_K64_curriculum_self_step1385.sh`
# (same ckpt, curriculum ON).
...
CFG=inference_sparse_K64_step1385_NOCURR_salad_n6_nfe400
