#!/bin/bash
# Designability probe for the K=40 curriculum+self ckpt with curriculum FORCED
# OFF at inference. Tests the trained model on the plain-K=40 path
# (n_seq=8, n_spatial=8, n_random=16 at all t).
#
# Distribution-shift caveat: the ckpt was trained against the curriculum
# schedule (20,0,0)/(14,4,8)/(8,8,16); the low-t bucket was sequential-only
# during training, so feeding it plain (8,8,16) at low t is a foreign input
# distribution. Numbers here reflect that mismatch, not a clean architectural
# comparison.
#
# N=6 × L∈{50, 100, 200} × nsteps=400. ~10-15 min on 1× L4.
...
CFG=inference_sparse_K40_curriculum_self_step1133_NOCURRIC_n6_nfe400
