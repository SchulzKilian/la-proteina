#!/bin/bash
# Always-denoised neighbors variant: threshold=1.0, so the sparse neighbor list
# is built from previous-step x_sc at EVERY step (after the step-0 bootstrap
# fills x_sc). Same canonical sparse-K40 ckpt (step 1259) as
# probe_sparse_K40_step1259_scoverride.sh — only the threshold changes.
#
...
CFG=inference_sparse_K40_step1259_scoverride_t1_n6_nfe400
