#!/bin/bash
# E085: inference-only Fix C2 t<0.4 on canonical sparse-K40 step 1259, with
# sc_neighbors_bootstrap=False. Predicted null (5/18 unchanged) vs E082's t<0.4
# arm (bootstrap=True default). Cost: ~17 min on 1× L4.
...
CFG=inference_sparse_K40_step1259_scoverride_nobootstrap_n6_nfe400
...
grep -E "\[sc_neighbors override\]|\[Fix C2\]" "${LOGBASE}.gen.log" || \
    echo "  WARNING: no Fix C2 / override canary found"
