#!/bin/bash
# Designability probe for the cold-start retrain of the FIVE-AXIS bundle:
#   K=64 SALAD-canonical sparse + curriculum + self-inclusion
#   + BigBird globals (n=4) + pair-update every 3 layers
#   + LOWTSOFT low-t bucket (16, 8, 24) + off-by-one cap fix.
#
# Targets whichever ckpt the repo-root symlink `sparse_K64_bigbird_lowtsoft_step1133.ckpt`
# points to. To probe a different step:
#   ln -sf /home/ks2218/la-proteina/store/ca_only_sparse_K64_curriculum_self_bigbird_pairupdate_lowtsoft/1778520282/checkpoints/best_val_<E>_<S>.ckpt \
#       /home/ks2218/la-proteina/sparse_K64_bigbird_lowtsoft_step1133.ckpt
# then re-run this script.
#
# N=6 × L∈{50, 100, 200} × nsteps=400. ~30 min on 1× A100.
# Inference config: configs/inference_sparse_K64_bigbird_lowtsoft_step1133_n6_nfe400.yaml.
...
# `rm -rf` only touches the nested same-named DIRECTORY; the sibling .pdb FILE
# is safe (different path, guarded by `[ -d … ]`).
