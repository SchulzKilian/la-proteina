"""Training loop + evaluation for the CA-conditioned multi-task property predictor.

Mirrors `multitask_predictor.train` (same optimizer / schedule / early-stop /
CSV layout). Two surface differences:

  * the model forward takes `coords` alongside `latents`
  * the saved checkpoint carries an extra `arch_kwargs` blob so the steering
    side can rebuild the exact PropertyTransformerCoords without guessing hyperparams.
"""
...
from .model_coords import PropertyTransformerCoords

logger = logging.getLogger(__name__)


def _arch_kwargs(config: dict) -> dict:
    """Pull the PropertyTransformerCoords constructor kwargs from the run config."""
    return {
        ...
        "n_rbf": config.get("n_rbf", 32),
        "rbf_max_nm": config.get("rbf_max_nm", 8.0),
        "relpos_clamp": config.get("relpos_clamp", 32),
        "relpos_dim": config.get("relpos_dim", 32),
    }


def train_one_epoch(
    model: PropertyTransformerCoords,
    loader: DataLoader,
...
def train_fold(
    ...
    train_ds = PropertyDatasetCoords(train_records, prop_df, stats=stats)
    val_ds = PropertyDatasetCoords(val_records, prop_df, stats=stats)
    ...
    val_loader = DataLoader(
        val_ds, batch_sampler=val_sampler, collate_fn=collate_fn_coords,
        num_workers=2, pin_memory=True, persistent_workers=True,
    )

    arch = _arch_kwargs(config)
