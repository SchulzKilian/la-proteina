...
**Update 2026-05-13 — inference-only K-set swap variant tested and ruled out** ([E062](experiments.md#e062--hybrid-grad-routing-inference-time-test-of-the-per-query-routing-prior-on-11-sparse-step-1800-frozen_t05--every_m50-2026-05-13)). Before committing to the training-side experiment proposed in the first follow-up bullet, we tested the cheaper inference-only variant: monkey-patch §11 sparse (step 1800) at sampling time so its `_build_neighbor_idx` consults the gradient-derived per-residue K-set from dense, leaving sparse's trained weights untouched. Two K-set update strategies were tried — `frozen_t05` (single K-set at anchor t=0.5, frozen for 400 steps) and `every_M=50` (refresh every 50 ODE steps at the consistent actual integrator-t, fixing the anchor mismatch the user flagged). Both gave **net negative paired scRMSD vs §11's content-free K-set baseline at N=9**: frozen_t05 mean Δ +2.23 Å (7 baseline wins / 2 hybrid / 0 ties), every_M=50 mean Δ +1.15 Å (5 / 3 / 1). The anchor-fix moves the needle ~1 Å in the right direction but does not flip the sign. **The most informative single observation is L=50 sample 0**, where the same noise seed produces hybrid 8.04 / 8.12 Å vs baseline 2.13 / 1.46 ✓ Å under the two strategies; the hybrid number is essentially invariant to anchor strategy, so the K-set anchor is NOT what drives the failure on that sample. **Mechanism (consistent with F12's predicted limitation):** §11's q/k/v projections, pair-bias entries, and t-dependent curriculum K-mix were calibrated for the (seq + spatial-1/d³ + random) K-set source during training; inference-time substitution of a different K-set — even a loss-aligned one drawn from dense's gradient saliency — is a distribution shift the trained weights have no calibration for. The downstream attention math computes q · k products and pair-bias additions at training-time magnitudes, but "neighbor j" now means something different. The result is a worse trajectory than even the content-free baseline K-set. **F12's narrow claim (per-query routing prior exists in dense; per-query gradient is a sharp/stable teacher signal) is unaffected** — F12 measured properties of the *teacher*, not whether the *student* can absorb them at inference time. **F12's constructive implikation narrows:** the per-query routing distillation must be **training-side** (the first follow-up bullet above), not inference-only K-set swap. The "K-as-a-function-of-N calibration" bullet and the "Per-query routing audit on a sparse-K64-curriculum-trained ckpt" bullet are unaffected by E062.
...
## Finding 13 — Real CamSol intrinsic solubility from the Sormanni-lab web server confirms that noise-aware-ensemble latent steering raises actual solubility on the predictor's training target without measurable codesign cost (2026-05-21; full seed-paired pH-7 addendum 2026-06-01)
...
### Addendum (2026-06-01) — Full seed-paired pH-7 CamSol batch resolves the population-vs-paired caveat and gives a standard-deviation-free headline

Vendruscolo returned CamSol (pH 7) for the **full 6419-sequence submission** spanning all weight cells — not just the 296-sequence / 2-cell web-server check above. Joined back via `camsol_submission_full_2026_05_27.index.tsv` (key = unique submission header; `script_utils/join_camsol_results.py`, consolidated `results/camsol_ph7_full_2026_05_27.csv`). Lab-notebook detail: [E116](experiments.md#e116--external-camsol-validation-of-the-solubility-steering-sweeps-vendruscolo-ph7-2026-06-01). This **resolves caveat #1** (population-level vs paired): the seed+length-matched unguided control `results/sanity_unsteered_seed42_45/unguided` (the 48 PDBs flagged as missing from the original submission) *was* in this batch, so every steered protein is now compared to its **same-seed, same-length** unguided counterpart.

**1. The web-server check and the full batch agree — the result is not an artifact of either run.** On the *identical* w32 / w128 proteins, the May web-server *intrinsic* CamSol and this *pH-7* batch correlate at **Pearson r = 0.90 (w32), 0.95 (w128)** (pH-7 runs ~0.18 higher systematically). The original Finding's "+0.65 at w32" and this addendum's "+0.94 at w32" are the **same effect measured against two different baselines**: the original used the *unmatched* stratified set (mean 1.733), this uses the *seed+length-matched* control (1.624, lower because it includes the low-CamSol L=500 cells). Same-baseline, pH-7 Δ = +0.78 vs intrinsic +0.65 (residual = the pH-7 offset). Nothing inflated; the matched baseline is simply the correct (and slightly lower) denominator.

**2. Full seed-paired dose-response (real CamSol units, length-binned), camsol_max:**

| w | baseline → steered (pooled CamSol) | ΔCamSol | % change | **proteins improved** | codesign (base 43.3%) |
|---|---|---|---|---|---|
| 8 | 1.62 → 1.80 | +0.17 | +11% | 40/48 (83%) | 40% |
| 16 | 1.62 → 1.98 | +0.35 | +22% | 44/48 (92%) | 40% |
| **32** | 1.62 → 2.56 | **+0.94** | **+58%** | **48/48 (100%)** | **42% ✓** |
| 48 | 1.62 → 3.54 | +1.92 | +118% | 48/48 (100%) | 27% ✗ |
| 64 | 1.62 → 4.72 | +3.10 | +191% | 48/48 | 19% ✗ |
| 128 | 1.62 → 7.01 | +5.38 | +331% | 48/48 | 2% ✗ |

**3. Standard-deviation-free headline (the supervisor-safe framing).** At the codesign-preserving operating point **w = 32**: **all 48 seed-matched proteins (16 seeds × 3 lengths) are more soluble by CamSol than their unsteered counterpart** — 48/48, **Wilson 95% CI ≥ 93%** — with a **median/mean gain of +0.94 CamSol units (+58%)** and codesignability statistically flat (41.7% vs 43.3%). This is *not* a small-n knife-edge: the **worst single protein still improved by +0.092 units**, so the 100% is a margin result, not 48 coin-flips. (For readers who object to a bare 100%, the bulletproof phrasing is "≥ 93% of proteins more soluble, 48/48 observed.") This is the σ-free replacement for the original Finding's per-length Cohen's-d table.

**4. The gain is genuine and non-circular, not gradient hacking.** The in-loop target was a *learned* CamSol head (5-fold `multitask_t1_noise_aware`, **held-out R² = 0.91 on all folds**, trained on real CamSol labels); the *actual* CamSol algorithm — which never entered the generation loop — independently confirms the move on the novel, off-distribution steered sequences. This is the **opposite of the E109/Finding-17 hackable case** (there predictor↔reality decoupled r 0.84→0.43; here they agree at r 0.90), placing CamSol in the *honest/linear-like* class with net_charge (Finding 16). The mechanism is **charged-residue enrichment** (D+E+K+R fraction 26.3% baseline → 30.1% at w32, still natural-range, → 45.1% at w128, degenerate) — which is the **correct biophysics**, since solubility is composition-driven; CamSol being a composition-based score makes its agreement appropriate, not tautological. The foldable ceiling is **w ≤ 32**: past it the composition shift becomes degenerate and codesignability collapses (27% → 19% → 2%).

**5. CamSol vs TANGO — comparably effective, different profiles.** Both objectives drive ~100% of seed-matched proteins past baseline at their codesign-preserving ceiling. **CamSol is more weight-efficient** (100% of proteins improved by w=32; TANGO needs w=48). **TANGO is more steering-robust** (keeps codesign 38% at w48, even 46% > baseline at w32; CamSol's foldability already breaks by w48). Note the original in-house audit *understated* CamSol steering (it tracked the SWI proxy, +1% at w32, because `camsol_intrinsic` was NaN) while TANGO's column was real (−14% at w32); the external CamSol corrects that impression — solubility steering was as effective as aggregation steering all along, just invisible to the SWI-tracking audit.

**Updated narrow claim (supersedes the population-level version above).** On the official LD3+AE2 checkpoint with the NA-v1 5-fold noise-aware ensemble, **seed-paired** latent steering at the codesign-preserving weight w = 32 makes **48/48 generated proteins (95% CI ≥ 93%) more soluble by real CamSol (pH 7)** than their same-seed unguided counterpart, median +0.94 units (+58%), at no measurable codesignability cost (41.7% vs 43.3%) — confirmed by the independent CamSol algorithm against a learned predictor (R² = 0.91), via the expected charged-composition mechanism, within the foldable range w ≤ 32. Remaining caveats from the parent Finding that still stand: CamSol is a predictor not a wet-lab assay (caveat #5/#7), the claim is within this design (generated proteins, L ∈ {300,400,500}, seeds 42–57), and the L=500 cells show the smallest gains.
...
## Finding 17 — Gradient-steering hackability is governed by predictor *separability* (additive vs windowed/saturating), not by the property or the steering direction; a supervised amino-acid concept bottleneck repairs the hackable case (2026-05-31)

**Experiment.** Three latent-steering results, read together:
- **`net_charge` target=−5** (additive predictor) — honest. [E105](experiments.md#e105--single-objective-steering-sweep-net_charge-target--50-setpoint-regulation--paired-designability-test-vs-seed-matched-unguided-2026-05-30) / [Finding 16](#finding-16).
- **`iupred3_fraction_disordered` target=0.123** (windowed/saturating predictor, steered *down* against the model's over-disordered prior) — gradient-hacked. [E109](experiments.md#e109--single-objective-steering-sweep-iupred3_fraction_disordered-target--0123-natural-setpoint-down-regulation-2026-05-31).
- **The fix:** a concept-bottleneck predictor `latent → (amino-acid identity + torsion angles) → 14 properties` (`CBMPropertyTransformer`, trained on the same `processed_latents_300_800` / `data/properties.csv` as the NA-v1 ensemble, 5-fold; best fold_2 r²_mean=0.934), swapped into `SteeringPredictor` as a drop-in and re-run on the *same* iupred w32 cell (seeds 42–57 × L{300,400,500}, nsteps=400, `inference_ucond_notri_long`). Lab detail: [E110](experiments.md#e110--concept-bottleneck-cbm-property-predictor-aatorsion-bottleneck-anti-goodhart-2026-05-31).

**Numbers.**
- **iupred down-regulation, NA-v1 vs CBM (matched seed/L/w/recipe, only the predictor changed):** both regulate the *predicted* value to the 0.121 setpoint; the *real* IUPred3 diverges. NA-v1: real **0.180** (UP from the ~0.158 prior — the *wrong* direction; per-sample pred↔real Pearson collapses 0.84→0.43 across w8→w32). CBM: real **0.128** (mean) / **0.072** (median) — DOWN toward the 0.123 target, 75% of proteins below the prior, 60% at/below target. Codesign: CBM 22/48 = **46%** vs NA-v1 43.8% vs unsteered ~47.9% (break-even — the fix is free).
- **Predictor↔real gap after steering:** systematic gap (real−pred) shrinks **+0.070 (NA-v1) → +0.014 (CBM)** — the CBM removes the calibration bias that made NA-v1 land real at 0.180. (Per-protein |gap| stays ~0.10 for both: at w32 both squeeze the predicted side to the setpoint, so neither resolves an individual protein's real disorder — the CBM win is aggregate calibration + correct direction, not per-sample precision.)
- **CBM accuracy cost ≈ 0:** vs the no-bottleneck t1 predictor (same data/regime) R²_mean 0.895→0.896; CBM ≥ on 13/14 properties; AA bottleneck 99.6% accurate.

**Narrow claim.** Across the tested cases, whether single-objective latent steering delivers the *real* property or gets gradient-hacked is predicted by **predictor separability**, not by which property or which direction:
- An **additive/separable** predictor (`net_charge` = Σ per-residue charges; gradient w.r.t. each residue is a context-free constant) has a **globally correct gradient** and stays honest **even when steered against the model's prior** (target −5 vs an ≈−10/−14 acidic prior — E105).
- A **non-separable, saturating** predictor (`iupred3` = logistic of a ~21-residue windowed energy, thresholded into a fraction) has a gradient that is only **locally valid (on the training manifold)**. Steered *with the grain* it stays honest in direction (`iupred_max` E067 even under-promises; `tango_min` E066 moves the right way); steered *against the prior*, off-manifold (`iupred` down to 0.123 from a 0.158 prior; off-manifold confirmed by the re-encode test E109e), its gradient **inverts** and real disorder rises.
- Replacing the latent→property shortcut with a **supervised amino-acid concept bottleneck** repairs the hackable case: routing the gradient through AA identity forces "lower predicted disorder" to mean "predict a less-disorder-prone composition," which genuinely *is* less disordered. The previously-reversed iupred down-regulation then succeeds (real 0.180→0.128) at no designability cost.

**Implikation.** Cautiously: the relevant axis for steering robustness is **how the property depends on the sequence**, not the property's chemistry or the steering sign. *Additive/composition-only* properties (net_charge, and by the same logic SWI/camsol — both per-residue sums) are safe to steer in any direction with a plain latent predictor; *context-dependent/saturating* properties (disorder, aggregation) are safe only while steering stays near the model's manifold, and become adversarially exploitable when pushed against the prior. The constructive consequence is that the failure is a property of the **predictor's parameterization, not of the task**: a concept bottleneck that grounds gradients in the property's true determinant (here, amino-acid composition) restores honesty without retraining the generator and without an accuracy penalty. This positions the CBM as the general anti-Goodhart lever for the context-dependent property class, complementing the noise-aware-ensemble lever of [Finding 10](#finding-10) (which hardens but does not eliminate the shortcut).

**Methodische Einschränkungen.**
- **The CBM steering fix is a single w=32 cell (n=48).** Direction-of-real-property and designability are decisive *vs E109's w32*, but the full **dose-response (w8–24)** — showing the CBM's pred↔real stays coupled where NA-v1's collapsed — is not yet run. The per-sample Pearson (CBM 0.21, NA-v1 0.43) is **not** evidence against the CBM: at w32 both predictors collapse the predicted side to the setpoint (restriction of range), so that statistic is uninformative; the substantive metric is the *direction* of the real property.
- **The separability framework is a post-hoc synthesis** over E105/E109/E066/E067/E028, not a prospective test. The clean confirmation is to steer a *new* additive property against-prior (predict honest) and a *new* windowed property against-prior (predict hackable). The CBM-driven `camsol_max` Pareto (E066 replication, running) is a partial test: camsol is additive + with-the-grain, so the CBM should *match* the NA-v1 frontier (no hack to fix); a sharp divergence would complicate the framework.
- **Real CamSol is NaN** in the developability panel, so camsol steering is read out on SWI (its real solubility proxy), as in F13/E066.
- **iupred unsteered baseline (~0.158)** is from E067/E099, not a seed-matched unguided arm in E110; the NA-v1-vs-CBM A/B is exact at matched (seed,L,w,recipe), so the 0.180→0.128 *direction flip* is clean regardless.
- **The CBM is NOT yet a clean swap for NA-v1 — it lacks NA-v1's two robustness levers.** NA-v1 = noise-aware (fine-tuned on flow-noised latents `z_t=(1−t)ε+t·z₁+0.1√(t(1−t))ε₂`, t∈[0.3,0.8]) **+ 5-fold ensemble**. The CBM here is **clean-z₁-trained (no noise injection) + single fold**. So vs NA-v1 the CBM adds the bottleneck but *removes* noise-injection and ensembling. Implications: (a) the iupred win is *stronger* than a like-for-like would suggest — the bottleneck alone, without either robustness lever, flipped the hack (so the AA bottleneck is an independent off-manifold-robustness mechanism, substituting for noise-injection by routing gradients through robustly-decodable AA); (b) the camsol weak-SWI-delivery is **confounded** — a clean-trained single fold is exactly the fragility noise-injection was designed to fix, so the gap may be the missing lever, not the bottleneck. The clean comparison (pending) is a **noise-aware 5-fold CBM ensemble** (fine-tune the folds with the same σ=0.1/t∈[0.3,0.8] injection) vs NA-v1 — ensemble-vs-ensemble, noise-aware-vs-noise-aware, only the bottleneck differing. (NB: *t-conditioning* itself is not the lever — the `fixt1` ablation matched NA-v1; the noised *data* is.)
- **Residual CBM caveats** (do not affect this finding's claim but bound it): (i) g2 is distribution-coupled to g1's soft output (feeding true one-hot AA breaks it — classic concept leakage; in-distribution at steering time); (ii) the CBM under-scores `swi` on an outlier subpopulation (full-val r²_swi 0.379 vs bulk 0.99 — a low-CV measurement artifact, not a steering issue); (iii) structural props (rg/sap/scm) cannot be grounded in an AA+torsion bottleneck since the latent lacks global geometry.

**Cross-references:**
- [E110](experiments.md#e110--concept-bottleneck-cbm-property-predictor-aatorsion-bottleneck-anti-goodhart-2026-05-31) — CBM build, R² vs no-bottleneck, the iupred steering re-run, re-encode/decomposition trail.
- [E109](experiments.md#e109--single-objective-steering-sweep-iupred3_fraction_disordered-target--0123-natural-setpoint-down-regulation-2026-05-31) — the gradient-hack this repairs (dose-response decoupling + re-encode localization).
- [E105](experiments.md#e105--single-objective-steering-sweep-net_charge-target--50-setpoint-regulation--paired-designability-test-vs-seed-matched-unguided-2026-05-30) / [Finding 16](#finding-16) — the additive-predictor control (honest against-prior).
- [Finding 10](#finding-10--closing-the-gradient-hacking-gap-in-latent-flow-steering-noise-aware-predictor-training--fold-ensembling-validated-by-real-property-delivery-and-structural-integrity-2026-05-06-codesignability-addendum-2026-05-07) — the prior (training-time) anti-hacking lever this complements.
- [E066](experiments.md#e066--high-w-noise-aware-scout-w326412816-seedsboth-directions--reveals-pareto-frontier-in-codesign-vs-property-2026-05-14) / [E067](experiments.md#e067--iupred3_fraction_disordered_max-scout-disorder-as-a-design-target-2026-05-15) — the camsol/tango/iupred-up cases the framework classifies.

---

## Finding 18 — The 8-dim latent is a near-pure per-residue *identity* code that organizes amino acids by intrinsic chemistry (size, hydrophobicity) but not by structural tendency (helix propensity) or charge as geometric axes (2026-06-01)

**Experiment.** Two probes of how the AE2 8-dim per-residue latent (`mean`, `processed_latents_300_800`) represents the 20 amino acids. (1) **AA-centroid clustering** ([E114](experiments.md#e114--aa-latent-clustering-does-the-8-dim-latent-organize-amino-acids-by-biochemistry-2026-06-01)): mean 8-d vector per AA over 2000 proteins / 781K residues; Ward clustering, nearest-latent-neighbour class purity, and leave-one-out (held-out *amino acid*, n=20) ridge probes of four biochemical axes. (2) **Residue-level large-n probe** ([E115](experiments.md#e115--residue-level-large-n-biochemical-axis-probe-held-out-protein-split-2026-06-01)): per-residue ridge from the latent, held-out-*protein* split (3000 train / 1000 test proteins, 1.56M residues), bootstrap-over-test-proteins 95% CI. Builds on the interpretability result that residue identity is 99.4%-decodable from the latent while spatial context is not (`dist_to_com` R²=0.008, `neighbour_count` R²=0.036), and on [Finding 3](#finding-3--latent-geometry-of-the-partial-autoencoder-is-well-utilized-and-locally-disentangled-2026)'s observation that dims 3 & 7 are multimodal (categorical) axes.

**Numbers.**
- **Residue-level test R² (held-out 1000 proteins / 392K residues, bootstrap 95% CI ±0.003):** residue volume **0.543** [0.539, 0.546]; KD hydrophobicity **0.503** [0.500, 0.505]; net charge **0.387** [0.383, 0.390]; helix propensity **0.263** [0.259, 0.266].
- **AA-centroid LOO R² (held-out amino acid, n=20):** volume **0.40**, hydrophobicity **0.37**, net charge **0.12**, helix propensity **−0.10**.
- **Clustering:** nearest-neighbour biochemical-class purity 35% (vs ~14% chance); Ward k=6 recovers aliphatic {I,L,V}, aromatic {F,W,Y}, smallest {A,G}, amide/acid pair {E,Q}. The carboxylate↔carboxamide twins collapse: Asn↔Asp are mutual nearest latent neighbours, and {E,Q} co-cluster — pairs that differ *only* in charge (identical hydrophobicity, near-identical volume). β-branched (1/3), hydroxyl (0/3) and helix-former (3/7) residues do **not** form latent clusters.

**Narrow claim.** From the per-residue latent, a linear probe predicts residue volume (R²=0.54), hydrophobicity (0.50), net charge (0.39) and helix propensity (0.26) on held-out proteins (n=392K, CI ±0.003). Volume and hydrophobicity are encoded as genuine latent-geometry axes — they also generalize across held-out amino acids (LOO R²=0.40/0.37). Net charge and helix propensity are recoverable only through amino-acid identity, **not** as organizing directions of the latent (centroid-LOO R²=0.12 / −0.10). Equivalently: the carboxylate-vs-carboxamide distinction (charge) is nearly invisible to the latent geometry, which is why Asp/Asn and Glu/Gln collapse.

**Implikation (cautious, separate from the narrow claim).** The AE2 latent behaves as a *physicochemical-identity* code: it captures the properties that intrinsically define a residue's sidechain (size, hydrophobicity, aromaticity — the things visible without folding context) and is geometrically blind to context-dependent tendencies (secondary-structure propensity) and largely to charge. This mirrors the spatial-blindness result (identity 99.4% decodable, neighbour-count R²=0.04) and confirms Finding 3's dims-3/7 categorical hypothesis (the two multimodal dims are the dominant identity-encoding axes). For steering design it predicts the boundary already seen empirically in Finding 4: properties that are sums of intrinsic per-residue chemistry (charge, hydrophobicity-driven) are cheaply latent-decodable, while spatial/structural properties require attention and are not single-latent-axis steerable.

**Methodische Einschränkungen.**
- The residue-level R² is *ceilinged by* identity-decodability (99.4%) and is dominated by it — it measures linear *readability* of a property per residue (identity-decode → lookup), not an identity-independent continuous axis. The centroid-LOO is the cleaner "is there a property axis" test; quote residue-level R² for "the latent lets you read off property X" and centroid-LOO for "the latent has a property-X axis". For charge/helix the two genuinely disagree.
- AA-centroid probes are intrinsically n=20 (only 20 amino acids); their R² cannot escape a small-sample caveat — that is *why* the residue-level companion exists.
- Biochemical scalars are standard literature values (Kyte-Doolittle, Zamyatnin volume, Pace-Scholtz helix), not dataset-derived; net_charge uses H=+0.1, so its number is sensitive to that choice.
- AE2 latents, length 300–800, single AE checkpoint; not shown to transfer to AE1 or other length regimes.
- Lab-notebook detail: [E114](experiments.md#e114--aa-latent-clustering-does-the-8-dim-latent-organize-amino-acids-by-biochemistry-2026-06-01) (clustering), [E115](experiments.md#e115--residue-level-large-n-biochemical-axis-probe-held-out-protein-split-2026-06-01) (large-n probe). Refines [Finding 3](#finding-3--latent-geometry-of-the-partial-autoencoder-is-well-utilized-and-locally-disentangled-2026) and [Finding 4](#finding-4--capacity-probing-separates-properties-by-probe-family-not-probe-size-2026-04-21).

---

## Future Experiment Ideas

- **CBM iupred dose-response (w∈{8,16,24,32}):** confirm the CBM's per-sample pred↔real coupling stays high where NA-v1's collapsed (0.84→0.43). The decisive completion of Finding 17.
- **Prospective test of the separability framework:** steer one *new additive* property and one *new windowed/saturating* property, each against the model's prior — predict honest vs hackable respectively. This converts the post-hoc synthesis into a falsifiable prediction.
- **CBM analytic heads for closed-form properties** (swi/net_charge/shannon/pI = Σ per-residue): replace the learned g2 head with the exact formula off the AA bottleneck (validated ceiling ~0.99) — eliminates the swi outlier-subpopulation artifact and is unhackable by construction.
- **Characterize the CBM swi outlier subpopulation** (full-val r²_swi 0.379 vs bulk 0.99): per-protein swi-error vs g1 AA-accuracy / length, to see if it's OOD proteins where the bottleneck's AA prediction degrades.
- **Sharpen / de-leak the g2 bottleneck:** temperature-anneal or straight-through the soft AA, or train g2 with concept dropout / on hard one-hot, so it reads identities rather than g1's soft texture (the test-(c) coupling).
